...
if (!JWT_SECRET) {
  console.error("Falta JWT_SECRET en el .env (obligatorio para sesiones seguras).");
  process.exit(1);
}
...
type SessionPayload = { userId: number; correo: string; rol: string; iat?: number; exp?: number };
...
app.get("/login", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "login.html")));
app.get("/register", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "register.html")));
app.get("/recover", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "recover.html")));
app.get("/product", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "product.html")));
app.get("/cart", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "cart.html")));
app.get("/confirm", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "confirm.html")));
app.get("/stores", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "store.html")));
app.get("/history", (_req: Request, res: Response) => res.sendFile(path.join(publicPath, "history.html")));
...
app.post("/api/auth/register", authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (contrasena.length < 8) return res.status(400).json({ message: "La contraseña debe tener mínimo 8 caracteres" });
    ...
    return res.status(201).json({ message: "Registro exitoso", userId: userInsert?.id });
  } catch (err) {
...
app.post("/api/auth/login", authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!correo || !contrasena) return res.status(400).json({ message: "Correo y contraseña son obligatorios" });
    if (!isValidEmail(correo)) return res.status(400).json({ message: "Correo inválido" });
    ...
    if (stored.startsWith("$2")) {
      ...
      if (ok) {
        try {
          ...
          await supabase.from("usuarios").update({ Contrasena: newHash }).eq("id", (user as any).id);
        } catch (rehashErr) {
    ...
    if (formError) console.error("[POST /api/auth/login] error formulario:", formError);
...
app.get("/api/auth/me", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (formError) console.error("[GET /api/auth/me] error formulario:", formError);
...
app.get("/api/landing/hero", async (_req: Request, res: Response) => {
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({ message: "Error al obtener hero", detail: error.message });
    }
...
app.get("/api/landing/about", async (_req: Request, res: Response) => {
  try {
    const { data, error } = await supabase
      .from("landing_about")
      .select("id, title, tagline, body, image_url, badge_text, cta_text, cta_href, instagram_handle")
    ...
    if (error) {
      ...
      return res.status(500).json({ message: "Error al obtener sección about", detail: error.message });
    }
...
app.get("/api/landing/instagram", async (_req: Request, res: Response) => {
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({ message: "Error al obtener historias de Instagram", detail: error.message });
    }
    return res.json(data || []);
  } catch (err) {
...
app.get("/api/pedidos", async (req: Request, res: Response) => {
  try {
    ...
    if (!correo) return res.status(400).json({ message: 'El parámetro "correo" es obligatorio' });
    ...
    if (!data || data.length === 0) return res.status(404).json({ message: "No se encontraron pedidos" });
...
app.post("/api/pedidos", async (req: Request, res: Response) => {
  try {
    const { nombre_cliente, resumen_pedido, direccion_cliente, celular_cliente, puntoventa, metodo_pago } = req.body;
...
app.patch("/api/pedidos/:id", async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await supabase.from("pedidos").update(updates).eq("id", id).select("id").maybeSingle();
